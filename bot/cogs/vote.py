    ...
    async def on_dbl_vote(self, data):
        if data["type"] == "upvote":
            ...
            Data.c.execute(
                ...
                {"new_amount": wallet + self.vote_reward, "user_id": user.id},
            )
            Data.conn.commit()

    @commands.command(
        name="vote",
        help="Get links to Bot List sites where you can vote for me",
    )
    async def vote(self, ctx):
        vote_embed = discord.Embed(
            title="Vote Bean Bot for Best Bot", color=self.theme_color
        )
        vote_embed.add_field(
            name="Top.gg",
            value="[Click here to vote](https://top.gg/bot/804243703455547462)",
        )
